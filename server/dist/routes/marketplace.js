...
router.post("/", users_1.isAuthenticated, upload.single("image"), async (req, res) => {
    try {
        const authHeader = req.headers.authorization;
        const token = authHeader?.split("Bearer ")[1];
        const supabase = (0, server_1.createClient)(token);
        const { name, price, condition, size, type, description, town, detailedAddress, } = req.body;
        if (!name ||
            !price ||
            !condition ||
            !size ||
            ...
            return res.status(400).json({
                success: false,
                error: "Please provide all required fields including town and detailed address",
            });
        }
        if (!req.file) {
            return res.status(400).json({
