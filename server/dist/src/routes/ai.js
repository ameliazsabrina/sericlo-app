...
router.post("/redesign", users_1.isAuthenticated, upload.single("image"), async (req, res) => {
    ...
    try {
        ...
        const { data: { publicUrl }, } = supabase.storage.from("redesigns").getPublicUrl(fileName);
...
router.get("/:id", users_1.isAuthenticated, async (req, res) => {
    try {
        const user_id = req.user.id;
        const authHeader = req.headers.authorization;
        const token = authHeader?.split("Bearer ")[1];
